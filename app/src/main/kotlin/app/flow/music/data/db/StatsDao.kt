...
interface StatsDao {
    ...
    @Query("SELECT contentKey FROM song_stats WHERE playCount > 0 ORDER BY playCount DESC LIMIT :limit")
    fun observeMostPlayed(limit: Int): Flow<List<Long>>

    @Query(
        """
        SELECT contentKey FROM song_stats
        WHERE lastPlayedAtMs IS NOT NULL
